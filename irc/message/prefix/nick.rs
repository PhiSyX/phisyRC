	...
	loop {
		match state {
			...
			| State::First => match stream.consume_next()? {
				...
				| _ => {
					return Err(
						IrcMessagePrefixNickError::InvalidFirstCharacter,
					)
				}
			},
			...
			| State::AfterFirst => match stream.consume_next()? {
				...
				//
				// Il s'agit d'une erreur d'analyse.
				| _ => return Err(IrcMessagePrefixNickError::InvalidCharacter),
			},
		}
	}
...
impl From<InputStreamError> for IrcMessagePrefixNickError {
	fn from(_: InputStreamError) -> Self {
		Self::InputStream
	}
}

impl fmt::Display for IrcMessagePrefixNickError {
...
mod tests {
	...
	#[test]
	fn test_nick_empty() {
		assert_eq!(parse(""), Err(IrcMessagePrefixNickError::IsEmpty));
	}
	...
	fn test_nick_invalid_first_character() {
		...
		assert_eq!(
			output,
			Err(IrcMessagePrefixNickError::InvalidFirstCharacter)
		);
	...
	fn test_nick_invalid_character() {
		let input = "Phi@SyX";
		let output = parse(input);
		assert_eq!(output, Err(IrcMessagePrefixNickError::InvalidCharacter));
	}
}
