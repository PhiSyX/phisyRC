...
impl<'a, 'b> ParseTagsBuilder<'a, 'b> {
	pub(super) fn analyze(&mut self) -> Result<(), IrcMessageTagsError> {
		loop {
			match self.state {
				| ParseTagsState::Initial => match self.stream.consume_next()? {
				...
				| ParseTagsState::LeftKey => {
					match self.stream.consume_next()? {
						| codepoint if codepoint.is_newline() => {
							return Err(
								IrcMessageTagsError::InvalidCharacter {
						...
						| CodePoint::EOF => {
							return Err(IrcMessageTagsError::ParseError);
						}
						...
						| _ => return Err(IrcMessageTagsError::ParseError),
					}
				}
...
impl<'a, 'b> ParseTagsBuilder<'a, 'b> {
	pub(super) fn finish(self) -> Result<IrcMessageTags, IrcMessageTagsError> {
		Ok(IrcMessageTags(self.temporary_map))
	}
}
